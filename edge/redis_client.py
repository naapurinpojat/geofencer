...
class RedisClient:
    ...
    def __init__(self, host, port, topic, logger=None):
        self.logger = logger
        self.topic = topic
...
class RedisConsumer:
    ...
    def __init__(
        self, client: RedisClient, topic: str, consumer_group: str, consumer_name: str, logger=None
    ):
        self.client = client
        self.topic = topic
    ...
    def read_messages(self, count=1, block=None):
        ...
        messages_ret = []
        messages = self.client.redis.xreadgroup(
            self.consumer_group, self.consumer_name, {self.topic: ">"}, count=count, block=block
        )
        for mes in messages:
